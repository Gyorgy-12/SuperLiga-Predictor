...
The frontend now applies fixture data from the Worker as the source of truth. It loads:

- `/bootstrap-light` on startup.
- `/fixtures?fresh=1` shortly after boot to avoid stale edge cache during admin tests.

The local `src/data/fixtures.js` schedule is only a fallback seed.
