...
import { coordinatorLiveResultsCache, ensureCoordinatorAlarm } from '../services/coordinator.service.js';
...
export async function bootstrapLightPayload(env, opts = {}) {
  ...
  const livePromise = coordinatorLiveResultsCache(env)
    .then(pack => pack || getLiveSnapshot())
    .catch(error => ({ ...getLiveSnapshot(), source: 'bootstrap-live-cache-error', error: error?.message || String(error), degraded: true }));
  ...
  const rawLive = livePack?.results && typeof livePack.results === 'object' ? livePack.results : {};
  const live = filterPrematureLiveResults(rawLive, fixturesPack.fixtures || []);
