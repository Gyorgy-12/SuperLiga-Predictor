...
function topCardsHtml(){
  ...
  const byYellow=teams.filter(n=>fp[n].yellow>0).sort((a,b)=>fp[b].yellow-fp[a].yellow||a.localeCompare(b)).slice(0,10);
  const byRed=teams.filter(n=>(fp[n].red+fp[n].yellowRed+fp[n].yellowPlusRed)>0).sort((a,b)=>(fp[b].red+fp[b].yellowRed+fp[b].yellowPlusRed)-(fp[a].red+fp[a].yellowRed+fp[a].yellowPlusRed)||a.localeCompare(b));
  ...
  let out='';
  if(byYellow.length)out+='<section class="card"><h2 class="card-title">Legtöbb sárga lap &ndash; Top 10</h2><div class="wc-stat-list stat-match-list">'+byYellow.map((n,i)=>{const t=fp[n];return cardStatRow(i+1,n,YC+' '+t.yellow,t.yellow+' sárga lap')}).join('')+'</div></section>';
  if(byRed.length)out+='<section class="card"><h2 class="card-title">Piros lapot kapott csapatok</h2><div class="wc-stat-list stat-match-list">'+byRed.map((n,i)=>{const t=fp[n],total=t.red+t.yellowRed+t.yellowPlusRed,parts=[];if(t.yellowRed)parts.push(YRC+' '+t.yellowRed+'&times; kettős sárga');if(t.red)parts.push(RC+' '+t.red+'&times; direkt piros');if(t.yellowPlusRed)parts.push(YRC+RC+' '+t.yellowPlusRed+'&times; sárga+piros');return cardStatRow(i+1,n,RC+' '+total,parts.join(' &middot; ')||total+' piros lap')}).join('')+'</div></section>';
  return out;
}
