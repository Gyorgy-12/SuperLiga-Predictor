...
function enforcePostseasonSeed(){if(FROZEN_MODE)return false;let seed=currentPostseasonSeed();if(!LAST_POSTSEASON_SEED){LAST_POSTSEASON_SEED=seed;try{localStorage.setItem('superliga_postseason_seed_v1',seed)}catch(e){}return false}if(seed!==LAST_POSTSEASON_SEED){let had=Object.keys(KO_PRED||{}).length>0;KO_PRED={};LAST_POSTSEASON_SEED=seed;try{localStorage.setItem('superliga_postseason_seed_v1',seed)}catch(e){}if(had)savePred();return had}return false}function statEffForMatches(matches,preds,isKo){let out={pts:0,max:0,pct:0,exact:0,diff:0,outcome:0,miss:0};matches.forEach(m=>{let r=actualFor({id:m.id});if(!r||!(r.started||r.finished)||!validScore(r.h)||!validScore(r.a))return;out.max++;let g=isKo?gradeKoTip((preds||{})[m.id],r):gradeTip((preds||{})[m.id],r);out.pts+=g.pts;out[g.cat]=(out[g.cat]||0)+1});out.pct=out.max?+(out.pts/out.max*100).toFixed(2):0;return out}
