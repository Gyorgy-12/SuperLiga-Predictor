...
function syncSpacer(){let t=byId('topBar'),s=byId('spacer'),h=Math.ceil(t.getBoundingClientRect().height||t.offsetHeight||148);s.style.height=h+'px';document.documentElement.style.setProperty('--top-h',h+'px');document.documentElement.style.setProperty('--control-top',(h+2)+'px');let cw=byId('ctrlWrap'),ch=cw?Math.ceil(cw.getBoundingClientRect().height||cw.offsetHeight||0):0;document.documentElement.style.setProperty('--ctrl-h',ch+'px')}
...
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&!FROZEN_MODE){syncLiveResults({force:true}).then(()=>scheduleLiveSync())}});
...
applyFixtureOverrides();applyTeamElo();applyOddsFromWorker();pruneStaleKoPred(false);render();syncSoon();syncLiveResults({force:true}).then(()=>scheduleLiveSync());
